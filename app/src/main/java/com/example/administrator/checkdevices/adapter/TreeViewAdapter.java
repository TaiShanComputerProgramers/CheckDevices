...
public class TreeViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void setDisplayNodes(List<TreeNode> displayNodes) {
...
//        return displayNodes.get(position).getContent().getLayoutId();
        return this.viewBinders.get(0).getLayoutId();
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
       /* View v = LayoutInflater.from(parent.getContext())
        ...
        switch (viewType){
            case R.layout.grid_list_item:
                return viewBinders.get(1).provideViewHolder(v);
//                break;
...
//           if (viewBinder.getLayoutId() == displayNodes.get(position).getContent().getLayoutId())
           ...
           /* if(viewBinder.getLayoutId()==R.layout.item_dir)viewBinder.bindView(holder,position,displayNodes.get(position));
            else viewBinder.bindView(holder,position+1,displayNodes.get(position+1));*/
        }
    }
   ...
   /* public void refresh(DeviceSnapshot treeNode) {
        for(int i=0;i<displayNodes.size();i++){
            DeviceSnapshot deviceItem1=(DeviceSnapshot) displayNodes.get(i).getContent();
            if(deviceItem1.getDeviceNo().equals(treeNode.getDeviceNo())){
                deviceItem1.setValue(deviceItem1,treeNode);
                notifyItemChanged(i);
                break;
            }
        }
    }*/
    public void refresh(Device_t treeNode) {
        for(int i=0;i<displayNodes.size();i++){
            Device_t deviceItem1=(Device_t) displayNodes.get(i).getContent();
            if(deviceItem1.getDeviceNo().equals(treeNode.getDeviceNo())){
//                deviceItem1=treeNode;
                deviceItem1.setValue(deviceItem1,treeNode);
                notifyItemChanged(i);
                break;
            }
